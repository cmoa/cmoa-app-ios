...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        CIBorderedButton *imageView = [[CIBorderedButton alloc] initWithFrame:CGRectZero];
        imageView.contentMode = UIViewContentModeCenter;
        [self addSubview:imageView];
        self.imageView = imageView;
        [imageView addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    CGSize imageSize = [self.imageView imageForState:UIControlStateNormal].size;
    ...
    self.imageView.frame = CGRectMake(contentFrame.size.width - (34 + 8),
                                      75 - ((75 / 2.0) + 8.5),
                                      34,
                                      34);
...
- (void)setToast:(CRToast *)toast {
    ...
    if (toast.imageTint) {
        image = [toast.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
