...
static NSString *const kStrContinue = @"Tap to continue";
...
- (void)goToCoachMarkIndexed:(NSUInteger)index {
    ...
    id markCaption = [markDef objectForKey:@"caption"];
    CGRect markRect;
    BOOL showCutout = NO;
    ...
    if ([markCaption isKindOfClass:[NSString class]]) {
        self.lblCaption.text = (NSString *)markCaption;
    } else if ([markCaption isKindOfClass:[NSMutableAttributedString class]]) {
        self.lblCaption.attributedText = (NSMutableAttributedString *)markCaption;
    }
    [self.lblCaption sizeToFit];
    
    // Show cutout or proceed without
    if (showCutout == YES) {
        ...
        // If cutout hasn't been drawn yet, set the cutout to the center of first mark
        if (cutoutDrawn == NO) {
            cutoutDrawn = YES;
    ...
    if (self.enableContinueLabel) {
        if (markIndex == 0) {
            ...
            lblContinue.text = self.strContinue;
